...
from .synapse_variant_runtime import get_label_mapping
...
def compute_group_confusion_from_logits(
    ...
    lm = get_label_mapping()
    pred = logits.argmax(dim=1, keepdim=True)
    valid = valid_mask.bool()

    class_tp = np.zeros((lm.NUM_OUTPUT_CHANNELS,), dtype=np.float64)
    class_fp = np.zeros((lm.NUM_OUTPUT_CHANNELS,), dtype=np.float64)
    class_fn = np.zeros((lm.NUM_OUTPUT_CHANNELS,), dtype=np.float64)

    active_cond_channels = lm.get_active_conditional_output_channels(group_id)
    active_eval_channels = list(lm.FIXED_OUTPUT_TO_ORIGINAL.keys())
    active_eval_channels += list(active_cond_channels)
    active_eval_channels += [lm.OTHER_CHANNEL]
    ...
    cond_tp = np.zeros((lm.NUM_COND_SLOTS,), dtype=np.float64)
    cond_fp = np.zeros((lm.NUM_COND_SLOTS,), dtype=np.float64)
    cond_fn = np.zeros((lm.NUM_COND_SLOTS,), dtype=np.float64)
    for slot_idx in range(lm.NUM_COND_SLOTS):
        channel_idx = int(lm.COND_SLOT_1_CHANNEL + slot_idx)
        if channel_idx not in active_cond_channels:
            continue
        pred_mask = pred == channel_idx
        gt_mask = target == channel_idx
        tp, fp, fn = _binary_counts(pred_mask, gt_mask, valid)
        cond_tp[slot_idx] += tp
        cond_fp[slot_idx] += fp
        cond_fn[slot_idx] += fn
    ...
    return (
        ...
        cond_fp,
        cond_fn,
        np.asarray([merged_tp], dtype=np.float64),
        np.asarray([merged_fp], dtype=np.float64),
        np.asarray([merged_fn], dtype=np.float64),
    )
...
def build_validation_report(
    ...
    label_names = lm.original_label_name_lookup()

    fixed_class_dice = {
        label_names.get(int(original_label), f"label_{int(original_label)}"): float(class_dice[int(channel)])
        for channel, original_label in lm.FIXED_OUTPUT_TO_ORIGINAL.items()
    }
    fixed_class_iou = {
        label_names.get(int(original_label), f"label_{int(original_label)}"): float(class_iou[int(channel)])
        for channel, original_label in lm.FIXED_OUTPUT_TO_ORIGINAL.items()
    }
    ...
    for spec in lm.DYNAMIC_GROUP_SPECS:
    ...
    for original_label in range(1, lm.NUM_ORIGINAL_LABELS):
        if original_label in lm.FIXED_ORIGINAL_TO_OUTPUT:
            original31_dice.append(float(class_dice[int(lm.FIXED_ORIGINAL_TO_OUTPUT[original_label])]))
            continue
    ...
    merged_conditional_dice = {
        spec.short_name: float(
            (2.0 * merged_cond_tp[spec.group_id])
            / (2.0 * merged_cond_tp[spec.group_id] + merged_cond_fp[spec.group_id] + merged_cond_fn[spec.group_id])
        )
    ...
    active_values.append(float(class_dice[lm.OTHER_CHANNEL]))
    active_values.extend(cond_subclass_dice.values())

    return {
        "fixed_class_dice": fixed_class_dice,
        "fixed_class_iou": fixed_class_iou,
        "other_dice": float(class_dice[lm.OTHER_CHANNEL]),
        "other_iou": float(class_iou[lm.OTHER_CHANNEL]),
        ...
        "output_channel_dice_foreground": [float(class_dice[i]) for i in range(1, lm.NUM_OUTPUT_CHANNELS)],
        "summary": {
...
def empty_validation_accumulators() -> Dict[str, np.ndarray]:
    ...
    return {
        ...
        "class_fp": np.zeros((lm.NUM_OUTPUT_CHANNELS,), dtype=np.float64),
        "class_fn": np.zeros((lm.NUM_OUTPUT_CHANNELS,), dtype=np.float64),
        "cond_tp": np.zeros((lm.NUM_DYNAMIC_GROUPS, lm.NUM_COND_SLOTS), dtype=np.float64),
        "cond_fp": np.zeros((lm.NUM_DYNAMIC_GROUPS, lm.NUM_COND_SLOTS), dtype=np.float64),
        "cond_fn": np.zeros((lm.NUM_DYNAMIC_GROUPS, lm.NUM_COND_SLOTS), dtype=np.float64),
        "merged_cond_tp": np.zeros((lm.NUM_DYNAMIC_GROUPS,), dtype=np.float64),
        "merged_cond_fp": np.zeros((lm.NUM_DYNAMIC_GROUPS,), dtype=np.float64),
        "merged_cond_fn": np.zeros((lm.NUM_DYNAMIC_GROUPS,), dtype=np.float64),
    }
