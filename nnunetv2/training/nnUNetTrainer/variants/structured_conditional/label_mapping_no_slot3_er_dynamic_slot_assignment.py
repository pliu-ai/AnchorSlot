...
# Fixed output head channels for the ER-dynamic variant.
...
DYNAMIC_GROUP_SPECS: Tuple[DynamicGroupSpec, ...] = (
    ...
    # ER membrane/lumen are dynamic-only in this variant.
    DynamicGroupSpec(12, "G13", "ERInCondSlot", (17, 18), ("er_mem", "er_lum")),
)

SLOT_ASSIGNMENT_MEM_LUM = "mem_lum"
SLOT_ASSIGNMENT_SWAPPED = "swapped"
SLOT_ASSIGNMENT_RANDOM = "random"
VALID_SLOT_ASSIGNMENT_POLICIES = (
    SLOT_ASSIGNMENT_MEM_LUM,
    SLOT_ASSIGNMENT_SWAPPED,
    SLOT_ASSIGNMENT_RANDOM,
)


def _normalize_slot_assignment_policy(value: str) -> str:
    normalized = str(value).strip().lower()
    if normalized not in VALID_SLOT_ASSIGNMENT_POLICIES:
        raise ValueError(
            "NNUNET_STRUCTCOND_SLOT_ASSIGNMENT_POLICY must be one of "
            f"{VALID_SLOT_ASSIGNMENT_POLICIES}, got {value!r}."
        )
    return normalized


SLOT_ASSIGNMENT_POLICY = _normalize_slot_assignment_policy(
    os.environ.get("NNUNET_STRUCTCOND_SLOT_ASSIGNMENT_POLICY", SLOT_ASSIGNMENT_MEM_LUM)
)
SLOT_ASSIGNMENT_RANDOM_SEED = int(os.environ.get("NNUNET_STRUCTCOND_SLOT_ASSIGNMENT_RANDOM_SEED", "12345"))


def _swap_spec_slots(spec: DynamicGroupSpec) -> DynamicGroupSpec:
    if spec.num_slots < 2:
        return spec
    return DynamicGroupSpec(
        group_id=spec.group_id,
        short_name=spec.short_name,
        display_name=spec.display_name,
        original_labels=tuple(reversed(spec.original_labels)),
        subclass_names=tuple(reversed(spec.subclass_names)),
    )


def _build_policy_group_specs(
    base_specs: Tuple[DynamicGroupSpec, ...],
    policy: str,
    random_seed: int,
) -> Tuple[DynamicGroupSpec, ...]:
    if policy == SLOT_ASSIGNMENT_MEM_LUM:
        return base_specs

    if policy == SLOT_ASSIGNMENT_SWAPPED:
        return tuple(_swap_spec_slots(spec) for spec in base_specs)

    # RANDOM policy: deterministic per run (seeded), per-group coin flip.
    rng = np.random.default_rng(int(random_seed))
    out: List[DynamicGroupSpec] = []
    for spec in base_specs:
        do_swap = bool(rng.integers(0, 2))
        out.append(_swap_spec_slots(spec) if do_swap else spec)
    return tuple(out)


DYNAMIC_GROUP_SPECS = _build_policy_group_specs(
    DYNAMIC_GROUP_SPECS,
    policy=SLOT_ASSIGNMENT_POLICY,
    random_seed=SLOT_ASSIGNMENT_RANDOM_SEED,
)
...
def get_slot_assignment_report() -> Dict[str, object]:
    """
    Return active slot assignment metadata for reproducibility/logging.
    """
    return {
        "policy": SLOT_ASSIGNMENT_POLICY,
        "random_seed": SLOT_ASSIGNMENT_RANDOM_SEED,
        "group_slot_assignments": {
            spec.short_name: {
                ...
                "slot1": {
                    "original_label": int(spec.original_labels[0]),
                    "name": str(spec.subclass_names[0]),
                },
                "slot2": {
                    "original_label": int(spec.original_labels[1]),
                    "name": str(spec.subclass_names[1]),
                },
            }
            for spec in DYNAMIC_GROUP_SPECS
        },
