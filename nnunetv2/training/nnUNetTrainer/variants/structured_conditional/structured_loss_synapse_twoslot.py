...
class StructuredConditionalLoss(nn.Module):
    ...
    def _build_foreground_class_mask(active_conditional_slots: torch.Tensor, device: torch.device) -> torch.Tensor:
        ...
        class_mask = torch.zeros((b, lm.NUM_OUTPUT_CHANNELS), dtype=torch.bool, device=device)
        for channel in lm.FIXED_OUTPUT_TO_ORIGINAL:
            class_mask[:, int(channel)] = True
        class_mask[:, lm.COND_SLOT_1_CHANNEL] = active_conditional_slots[:, 0]
        class_mask[:, lm.COND_SLOT_2_CHANNEL] = active_conditional_slots[:, 1]
        class_mask[:, lm.OTHER_CHANNEL] = True
        return class_mask
    ...
    def _masked_ce(
        ...
        lm = get_label_mapping()
        target_labels = target[:, 0].long().clamp(min=0, max=lm.NUM_OUTPUT_CHANNELS - 1)
    ...
    def _masked_soft_dice_from_probs(
        ...
        lm = get_label_mapping()
        target_labels = target[:, 0].long().clamp(min=0, max=lm.NUM_OUTPUT_CHANNELS - 1)
        target_onehot = F.one_hot(target_labels, num_classes=lm.NUM_OUTPUT_CHANNELS)
    ...
    def _suppression_loss(
        ...
        lm = get_label_mapping()
        cond_probs = torch.stack((probs[:, lm.COND_SLOT_1_CHANNEL], probs[:, lm.COND_SLOT_2_CHANNEL]), dim=1)
        active = active_conditional_slots.float()
        active = active.view(active.shape[0], active.shape[1], *([1] * (probs.ndim - 2)))
        cond_region = ((target == lm.COND_SLOT_1_CHANNEL) | (target == lm.COND_SLOT_2_CHANNEL)).float()
