...
from .label_mapping_synapse_8a1s import (
    COND_SLOT_1_CHANNEL,
    DYNAMIC_GROUP_SPECS,
    LIVER_CHANNEL,
    SPLEEN_CHANNEL,
...
FIXED_CHANNEL_NAME_MAP = {
    LIVER_CHANNEL: "liver",
    SPLEEN_CHANNEL: "spleen",
    STOMACH_CHANNEL: "stomach",
    AORTA_CHANNEL: "aorta",
    IVC_CHANNEL: "ivc",
    PANCREAS_CHANNEL: "pancreas",
    RKIDNEY_CHANNEL: "r_kidney",
    LKIDNEY_CHANNEL: "l_kidney",
}

# Fixed original labels in Synapse/BTCV.
...
def compute_group_confusion_from_logits(
    ...
    # All 8 fixed anchors must be scored (not just the original 4); otherwise the
    # uncounted anchor channels read Dice 0 and deflate mean_fixed/mean_original31.
    active_eval_channels = list(FIXED_CHANNEL_NAME_MAP.keys())
    ...
    cond_tp = np.zeros((1,), dtype=np.float64)
    cond_fp = np.zeros((1,), dtype=np.float64)
    cond_fn = np.zeros((1,), dtype=np.float64)
    for slot_idx, channel_idx in enumerate((COND_SLOT_1_CHANNEL,)):
...
def build_validation_report(
    ...
    # Original Synapse foreground labels (1..13), ordered by original ID.
    # (Dict keys keep the *_original31_* names for trainer compatibility.)
    original31_dice: List[float] = []
    for original_label in range(1, NUM_ORIGINAL_LABELS):
...
def empty_validation_accumulators() -> Dict[str, np.ndarray]:
    ...
    return {
        ...
        "class_fp": np.zeros((NUM_OUTPUT_CHANNELS,), dtype=np.float64),
        "class_fn": np.zeros((NUM_OUTPUT_CHANNELS,), dtype=np.float64),
        "cond_tp": np.zeros((NUM_DYNAMIC_GROUPS, 1), dtype=np.float64),
        "cond_fp": np.zeros((NUM_DYNAMIC_GROUPS, 1), dtype=np.float64),
        "cond_fn": np.zeros((NUM_DYNAMIC_GROUPS, 1), dtype=np.float64),
