...
class StructuredConditionalLoss(nn.Module):
    """
    Structured loss for the fixed 11-channel output (8 anchors + 1 conditional slot).
    ...
    def _build_foreground_class_mask(active_conditional_slots: torch.Tensor, device: torch.device) -> torch.Tensor:
        """Foreground mask for Dice: 8 fixed anchors + the single cond slot + other."""
        b = int(active_conditional_slots.shape[0])
        class_mask = torch.zeros((b, NUM_OUTPUT_CHANNELS), dtype=torch.bool, device=device)

        # Fixed foreground channels (8 anchors at indices 1..8).
        class_mask[:, 1:9] = True

        # Single conditional slot.
    ...
    def _masked_ce(
        ...
        # Single conditional slot is always active for every group, so no slot
        # needs to be masked out of CE competition.
        logits_ce = logits
    ...
    def _suppression_loss(
        ...
        """
        probs = torch.softmax(logits, dim=1)
        cond_probs = probs[:, COND_SLOT_1_CHANNEL : COND_SLOT_1_CHANNEL + 1]

        active = active_conditional_slots.float()
        active = active.view(active.shape[0], active.shape[1], *([1] * (logits.ndim - 2)))

        cond_region = (target == COND_SLOT_1_CHANNEL).float()
