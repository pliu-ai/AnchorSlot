...
from .dataloader_structured_conditional_er_dynamic import StructuredConditionalDataLoader3D
from .inference_structured_conditional_er_dynamic import (
...
from .metrics_structured_conditional_er_dynamic import (
...
from .structured_loss_er_dynamic import StructuredConditionalLoss, StructuredLossConfig
from .text_contrastive import VisualTextContrastiveLoss


FIXED_ORIGINAL_LABEL_NAMES: Dict[int, str] = {
    1: "ecs",
    2: "pm",
    3: "cyto",
    27: "nucpl",
...
def _build_original_label_text_embedding_matrix(
    text_emb_path: str,
) -> Tuple[torch.Tensor, torch.Tensor, List[int]]:
    blob = _load_text_embedding_blob(text_emb_path)
    label_embeddings = blob.get("label_embeddings", {})
    anchor_embeddings = blob.get("anchor_embeddings", {})
    if not isinstance(label_embeddings, dict) or not isinstance(anchor_embeddings, dict):
        raise KeyError(f"{text_emb_path} must contain dict keys 'label_embeddings' and 'anchor_embeddings'")

    text_dim = _infer_text_embedding_dim(text_emb_path)
    matrix = torch.zeros((32, text_dim), dtype=torch.float32)
    valid_ids: List[int] = []
    missing_ids: List[int] = []

    for original_id, anchor_name in FIXED_ORIGINAL_LABEL_NAMES.items():
        value = anchor_embeddings.get(anchor_name)
        if value is None:
            missing_ids.append(int(original_id))
            continue
        matrix[int(original_id)] = torch.nn.functional.normalize(value.detach().float().reshape(-1), dim=-1)
        valid_ids.append(int(original_id))

    for spec in DYNAMIC_GROUP_SPECS:
        for original_id, label_name in zip(spec.original_labels, spec.subclass_names):
    ...
    return matrix, torch.as_tensor(valid_ids, dtype=torch.long), missing_ids
...
class nnUNetTrainerStructuredConditionalERDynamic(nnUNetTrainer):
    """
    ...
    default_class_balance_mode = "none"
    default_group_sampling_mode = "random"
    ...
    def __init__(
        ...
        self.group_sampling_mode = str(
            os.environ.get(
                "NNUNET_STRUCTCOND_GROUP_SAMPLING_MODE",
                getattr(self, "default_group_sampling_mode", "random"),
            )
        ).strip().lower()
        ...
        if self.group_sampling_mode not in {"random", "balanced"}:
            raise ValueError("NNUNET_STRUCTCOND_GROUP_SAMPLING_MODE must be 'random' or 'balanced'.")
        self.group_balance_power = float(os.environ.get("NNUNET_STRUCTCOND_GROUP_BALANCE_POWER", "1.0"))
        if self.group_balance_power < 0:
            raise ValueError("NNUNET_STRUCTCOND_GROUP_BALANCE_POWER must be >= 0.")
        ...
        self.loss_cfg = StructuredLossConfig(
            ...
            class_balance_mode=str(
                os.environ.get(
                    "NNUNET_STRUCTCOND_CLASS_BALANCE_MODE",
                    getattr(self, "default_class_balance_mode", "none"),
                )
            ).strip().lower(),
            class_balance_power=float(os.environ.get("NNUNET_STRUCTCOND_CLASS_BALANCE_POWER", "0.5")),
            class_balance_min_weight=float(os.environ.get("NNUNET_STRUCTCOND_CLASS_BALANCE_MIN_WEIGHT", "0.25")),
            class_balance_max_weight=float(os.environ.get("NNUNET_STRUCTCOND_CLASS_BALANCE_MAX_WEIGHT", "4.0")),
            class_balance_eps=float(os.environ.get("NNUNET_STRUCTCOND_CLASS_BALANCE_EPS", "1e-6")),
        )
        if self.loss_cfg.class_balance_power < 0:
            raise ValueError("NNUNET_STRUCTCOND_CLASS_BALANCE_POWER must be >= 0.")
        if self.loss_cfg.class_balance_min_weight <= 0:
            raise ValueError("NNUNET_STRUCTCOND_CLASS_BALANCE_MIN_WEIGHT must be > 0.")
        if self.loss_cfg.class_balance_max_weight < self.loss_cfg.class_balance_min_weight:
            raise ValueError("NNUNET_STRUCTCOND_CLASS_BALANCE_MAX_WEIGHT must be >= min weight.")
        if self.loss_cfg.class_balance_eps <= 0:
            raise ValueError("NNUNET_STRUCTCOND_CLASS_BALANCE_EPS must be > 0.")

        # Optional visual-text contrastive alignment. Defaults off and does not
        # alter the structured AnchorSlot loss unless explicitly enabled.
        self.use_text_contrastive = (
            str(os.environ.get("NNUNET_STRUCTCOND_USE_TEXT_CONTRASTIVE", "0")).lower()
            in {"1", "true", "yes", "y"}
        )
        self.text_contrast_weight = float(os.environ.get("NNUNET_STRUCTCOND_TEXT_CONTRAST_WEIGHT", "0.05"))
        self.text_contrast_tau = float(os.environ.get("NNUNET_STRUCTCOND_TEXT_CONTRAST_TAU", "0.1"))
        self.text_contrast_num_samples = int(os.environ.get("NNUNET_STRUCTCOND_TEXT_CONTRAST_NUM_SAMPLES", "40"))
        self.text_contrast_dim = int(os.environ.get("NNUNET_STRUCTCOND_TEXT_CONTRAST_DIM", "128"))
        self.contrast_all_present_labels = (
            str(os.environ.get("NNUNET_STRUCTCOND_CONTRAST_ALL_PRESENT_LABELS", "0")).lower()
            in {"1", "true", "yes", "y"}
        )
        self.text_contrast_include_anchors = (
            str(os.environ.get("NNUNET_STRUCTCOND_TEXT_CONTRAST_INCLUDE_ANCHORS", "1")).lower()
            in {"1", "true", "yes", "y"}
        )
        self.text_contrast_include_active_dynamic = (
            str(os.environ.get("NNUNET_STRUCTCOND_TEXT_CONTRAST_INCLUDE_ACTIVE_DYNAMIC", "1")).lower()
            in {"1", "true", "yes", "y"}
        )
        self.text_contrast_embedding_path = str(os.environ.get("NNUNET_STRUCTCOND_TEXT_EMB_PATH", "")).strip()
        if self.use_text_contrastive:
            if self.text_contrast_weight < 0:
                raise ValueError("NNUNET_STRUCTCOND_TEXT_CONTRAST_WEIGHT must be >= 0.")
            if self.text_contrast_tau <= 0:
                raise ValueError("NNUNET_STRUCTCOND_TEXT_CONTRAST_TAU must be > 0.")
            if self.text_contrast_num_samples < 1:
                raise ValueError("NNUNET_STRUCTCOND_TEXT_CONTRAST_NUM_SAMPLES must be >= 1.")
            if self.text_contrast_dim <= 0:
                raise ValueError("NNUNET_STRUCTCOND_TEXT_CONTRAST_DIM must be > 0.")
            if not self.text_contrast_embedding_path:
                raise ValueError(
                    "NNUNET_STRUCTCOND_USE_TEXT_CONTRASTIVE=1 requires NNUNET_STRUCTCOND_TEXT_EMB_PATH"
                )
        self.text_contrastive_loss = None
        self.text_embeddings_by_original_label = None
        self.valid_text_original_label_ids = torch.empty((0,), dtype=torch.long)
        self.active_group_label_mapping = {
            int(spec.group_id): tuple(int(i) for i in spec.original_labels)
            for spec in DYNAMIC_GROUP_SPECS
        }
        self._latest_text_contrast_stats = {
            "loss": 0.0,
            "num_samples": 0,
            "num_labels": 0,
            "sampled_label_ids": [],
        ...
        # Optional BioMedCLIP text-generated dynamic slot head. Defaults off;
        # when enabled, only channels cond_slot_1..cond_slot_3 are generated
        # from label text prototypes. Other structured channels still come from
        # the normal segmentation head.
        ...
        self.text_generated_slot_alpha = float(
            os.environ.get("NNUNET_STRUCTCOND_TEXT_GENERATED_SLOT_ALPHA", "1.0")
        )
        if self.use_text_generated_slot_head:
            ...
            if self.text_generated_slot_hidden_dim <= 0:
                raise ValueError("NNUNET_STRUCTCOND_TEXT_GENERATED_SLOT_HIDDEN_DIM must be > 0.")
        ...
        self._latest_text_generated_slot_stats = {
            "active_slots": 0,
            "inactive_slots": 0,
            "mean": 0.0,
            "std": 0.0,
        ...
        self._fallback_group_balance_counts = np.zeros((self.num_dynamic_groups,), dtype=np.int64)
    ...
    def _setup_wandb(self) -> None:
        ...
        run = wandb.init(
            ...
            config={
                ...
                "group_sampling_mode": self.group_sampling_mode,
                "grouping_config_path": self.grouping_config_path if self.grouping_config_path else None,
                "group_balance_power": self.group_balance_power,
                "class_balance_mode": self.loss_cfg.class_balance_mode,
                "class_balance_power": self.loss_cfg.class_balance_power,
                "class_balance_min_weight": self.loss_cfg.class_balance_min_weight,
                "class_balance_max_weight": self.loss_cfg.class_balance_max_weight,
                ...
                "use_text_contrastive": self.use_text_contrastive,
                "text_contrast_weight": self.text_contrast_weight,
                "text_contrast_tau": self.text_contrast_tau,
                "text_contrast_num_samples": self.text_contrast_num_samples,
                "text_contrast_dim": self.text_contrast_dim,
                "contrast_all_present_labels": self.contrast_all_present_labels,
    ...
    def _log_wandb_epoch(self) -> None:
        ...
        if self.use_text_contrastive:
            payload.update(
                {
                    "train/text_contrast_loss": float(self._latest_text_contrast_stats.get("loss", 0.0)),
                    "train/text_contrast_num_samples": float(
                        self._latest_text_contrast_stats.get("num_samples", 0)
                    ),
                    "train/text_contrast_num_labels": float(
                        self._latest_text_contrast_stats.get("num_labels", 0)
                    ),
                }
            )
        if self.use_text_generated_slot_head:
            payload.update(
                {
                    "train/text_generated_slot_active_slots": float(
                        self._latest_text_generated_slot_stats.get("active_slots", 0)
                    ),
                    "train/text_generated_slot_inactive_slots": float(
                        self._latest_text_generated_slot_stats.get("inactive_slots", 0)
                    ),
                    "train/text_generated_slot_logit_mean": float(
                        self._latest_text_generated_slot_stats.get("mean", 0.0)
                    ),
                    "train/text_generated_slot_logit_std": float(
                        self._latest_text_generated_slot_stats.get("std", 0.0)
                    ),
                }
            )
        if (
    ...
    def build_network_architecture(
        ...
        # Default path remains the original learned group embedding. The new
        # opt-in text-conditioning path fuses learned group embeddings with
        # precomputed BioMedCLIP group text embeddings; BioMedCLIP itself is not
        # loaded in training.
        ...
        use_text_contrastive = (
            str(os.environ.get("NNUNET_STRUCTCOND_USE_TEXT_CONTRASTIVE", "0")).lower()
            in {"1", "true", "yes", "y"}
        )
        text_contrast_dim = int(os.environ.get("NNUNET_STRUCTCOND_TEXT_CONTRAST_DIM", "128"))
        text_contrast_text_dim = None
        if use_text_contrastive:
            if not text_emb_path:
                raise ValueError(
                    "NNUNET_STRUCTCOND_USE_TEXT_CONTRASTIVE=1 requires NNUNET_STRUCTCOND_TEXT_EMB_PATH"
                )
            text_contrast_text_dim = _infer_text_embedding_dim(text_emb_path)
        ...
        if use_text_conditioning:
            ...
            text_emb_path = os.environ.get("NNUNET_STRUCTCOND_TEXT_EMB_PATH", "").strip()
            if not text_emb_path:
                raise ValueError("NNUNET_STRUCTCOND_CONDITION_MODE=text requires NNUNET_STRUCTCOND_TEXT_EMB_PATH")
            ...
            print(f"[StructuredConditional] condition_mode=text  text_emb={text_emb_path}  "
        ...
        return StructuredConditionalUNet(
            ...
            text_contrast_text_dim=text_contrast_text_dim,
            text_contrast_dim=text_contrast_dim,
    ...
    def _setup_text_contrastive(self) -> None:
        if not self.use_text_contrastive:
            return
        text_matrix, valid_ids, missing_ids = _build_original_label_text_embedding_matrix(
            self.text_contrast_embedding_path
        )
        self.text_embeddings_by_original_label = text_matrix.to(self.device, non_blocking=True)
        self.valid_text_original_label_ids = valid_ids.to(self.device, non_blocking=True)
        ignore_ids = [0]
        if self.label_manager.ignore_label is not None:
            ignore_ids.append(int(self.label_manager.ignore_label))
        self.text_contrastive_loss = VisualTextContrastiveLoss(
            text_contrast_tau=self.text_contrast_tau,
            text_contrast_num_samples=self.text_contrast_num_samples,
            text_contrast_include_anchors=self.text_contrast_include_anchors,
            text_contrast_include_active_dynamic=self.text_contrast_include_active_dynamic,
            contrast_all_present_labels=self.contrast_all_present_labels,
            ignore_label_ids=ignore_ids,
            anchor_label_ids=tuple(sorted(FIXED_ORIGINAL_LABEL_NAMES)),
        ).to(self.device)
        if missing_ids:
            self.print_to_log_file(
                "[TextContrast] warning missing BioMedCLIP text embeddings for original label ids: "
                + json.dumps(missing_ids)
            )
        self.print_to_log_file(
            "[TextContrast] enabled "
            f"path={self.text_contrast_embedding_path}, "
            f"weight={self.text_contrast_weight}, tau={self.text_contrast_tau}, "
            f"num_samples={self.text_contrast_num_samples}, dim={self.text_contrast_dim}, "
            f"contrast_all_present_labels={self.contrast_all_present_labels}, "
            f"valid_text_labels={self.valid_text_original_label_ids.detach().cpu().tolist()}"
        )

    def _compute_text_contrastive_loss(
        self,
        projected_feature_map: torch.Tensor,
        projected_text_embeddings: torch.Tensor,
        original_target_high: torch.Tensor,
        group_ids: torch.Tensor,
    ) -> Tuple[torch.Tensor, Dict[str, object]]:
        if self.text_contrastive_loss is None:
            zero = projected_feature_map.float().reshape(-1)[0] * 0.0
            return zero, {"num_samples": 0, "num_labels": 0, "sampled_label_ids": []}
        loss = self.text_contrastive_loss(
            feature_map=projected_feature_map,
            original_label_map=original_target_high,
            group_ids=group_ids,
            text_embeddings_by_label=projected_text_embeddings,
            valid_original_label_ids=self.valid_text_original_label_ids,
            active_group_label_mapping=self.active_group_label_mapping,
        )
        stats = dict(self.text_contrastive_loss.last_stats)
        return loss, stats
    ...
    def _sample_group_ids_from_target(self, target_high: torch.Tensor) -> torch.Tensor:
        ...
        for b in range(int(target_high.shape[0])):
            ...
            group_id = self._sample_group_id_for_case(sorted(present))
            sampled.append(int(group_id))
        return torch.as_tensor(sampled, dtype=torch.long, device=self.device)

    def _sample_group_id_for_case(self, present_group_ids) -> int:
        ...
        present_sorted = sorted({int(i) for i in present_group_ids if 0 <= int(i) < self.num_dynamic_groups})
        absent = [i for i in range(self.num_dynamic_groups) if i not in present_sorted]
        if len(present_sorted) == 0:
            candidates = list(range(self.num_dynamic_groups))
        elif bool(self.group_sampling_rng.random() < self.p_present_group):
        ...
        counts = self._fallback_group_balance_counts[np.asarray(candidates, dtype=np.int64)].astype(np.float64)
        weights = np.power(counts + 1.0, -self.group_balance_power)
        weights = weights / np.clip(weights.sum(), a_min=1e-12, a_max=None)
        group_id = int(self.group_sampling_rng.choice(np.asarray(candidates, dtype=np.int64), p=weights))
        self._fallback_group_balance_counts[group_id] += 1
        return group_id
    ...
    def on_train_start(self):
        ...
        self.print_to_log_file(
            ...
            f"group_sampling_mode={self.group_sampling_mode}, "
            f"grouping_config_path={self.grouping_config_path if self.grouping_config_path else None}, "
            f"group_balance_power={self.group_balance_power}, "
            ...
            f"class_balance_mode={self.loss_cfg.class_balance_mode}, "
            f"class_balance_power={self.loss_cfg.class_balance_power}, "
            f"class_balance_min_weight={self.loss_cfg.class_balance_min_weight}, "
            f"class_balance_max_weight={self.loss_cfg.class_balance_max_weight}, "
            ...
            f"use_text_contrastive={self.use_text_contrastive}, "
            f"text_contrast_weight={self.text_contrast_weight}, "
            f"text_contrast_tau={self.text_contrast_tau}, "
            f"text_contrast_num_samples={self.text_contrast_num_samples}, "
            f"text_contrast_dim={self.text_contrast_dim}, "
            f"contrast_all_present_labels={self.contrast_all_present_labels}, "
        ...
        for candidate in candidates:
            if hasattr(candidate, "set_epoch"):
                try:
                    ...
                    self.print_to_log_file(
                        f"[StructuredConditional] failed to set epoch on train loader: {e}"
                    )
                    return

    def on_train_epoch_start(self):
        super().on_train_epoch_start()
        self._fallback_group_balance_counts[:] = 0
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with self._autocast_context():
            if self.use_text_contrastive:
                output_pack = self.network(data, group_ids, return_features=True)
                output = output_pack["logits"]
                network_mod = self._unwrap_network()
                projected_feature_map = network_mod.visual_text_proj(output_pack["decoder_last"])
                projected_text_embeddings = network_mod.text_contrast_proj(self.text_embeddings_by_original_label)
            else:
        ...
        structured_loss = self._compute_structured_loss(loss_output, remapped_target, valid_mask, active_slots)
        text_contrast_loss = structured_loss.reshape(-1)[0] * 0.0
        text_contrast_stats: Dict[str, object] = {
            "num_samples": 0,
            "num_labels": 0,
            "sampled_label_ids": [],
            "missing_text_label_ids": [],
        ...
        if self.use_text_contrastive:
            if projected_feature_map is None or projected_text_embeddings is None:
                raise RuntimeError("text contrastive projections were not produced")
            text_contrast_loss, text_contrast_stats = self._compute_text_contrastive_loss(
        ...
        loss = structured_loss + float(self.text_contrast_weight) * text_contrast_loss
        ...
        return {
            ...
            "structured_loss": structured_loss.detach().cpu().numpy(),
            "text_contrast_loss": text_contrast_loss.detach().cpu().numpy(),
            "text_contrast_num_samples": int(text_contrast_stats.get("num_samples", 0)),
            "text_contrast_num_labels": int(text_contrast_stats.get("num_labels", 0)),
            "text_contrast_sampled_label_ids": list(text_contrast_stats.get("sampled_label_ids", [])),
            "text_contrast_missing_label_ids": list(text_contrast_stats.get("missing_text_label_ids", [])),
    ...
    def on_train_epoch_end(self, train_outputs: List[dict]):
        ...
        if self.use_text_contrastive and train_outputs:
            contrast_losses = [float(o.get("text_contrast_loss", 0.0)) for o in train_outputs]
            contrast_samples = [int(o.get("text_contrast_num_samples", 0)) for o in train_outputs]
            contrast_labels = [int(o.get("text_contrast_num_labels", 0)) for o in train_outputs]
            sampled_label_ids = sorted(
                {
                    int(label_id)
                    for output in train_outputs
                    for label_id in output.get("text_contrast_sampled_label_ids", [])
                }
            )
            missing_label_ids = sorted(
                {
                    int(label_id)
                    for output in train_outputs
                    for label_id in output.get("text_contrast_missing_label_ids", [])
                }
            )
            self._latest_text_contrast_stats = {
                "loss": float(np.mean(contrast_losses)) if contrast_losses else 0.0,
                "num_samples": float(np.mean(contrast_samples)) if contrast_samples else 0.0,
                "num_labels": float(np.mean(contrast_labels)) if contrast_labels else 0.0,
                "sampled_label_ids": sampled_label_ids,
                "missing_text_label_ids": missing_label_ids,
            }
            self.print_to_log_file(
                "[TextContrast] epoch summary "
                + json.dumps(self._latest_text_contrast_stats)
            )
...
class nnUNetTrainerStructuredConditionalERDynamicClassBalancedCE(
    nnUNetTrainerStructuredConditionalERDynamic
):
    """AnchorSlot with mild inverse-sqrt CE weights over the remapped 9-channel target."""

    default_class_balance_mode = "remap_sqrt"


class nnUNetTrainerStructuredConditionalERDynamicSlotBalancedCE(
    nnUNetTrainerStructuredConditionalERDynamic
):
    """AnchorSlot with inverse-sqrt CE weights only on active conditional slots."""

    default_class_balance_mode = "slot_sqrt"


class nnUNetTrainerStructuredConditionalERDynamicGroupBalanced(
    nnUNetTrainerStructuredConditionalERDynamic
):
    """AnchorSlot with group-balanced present/absent condition sampling."""

    default_group_sampling_mode = "balanced"
