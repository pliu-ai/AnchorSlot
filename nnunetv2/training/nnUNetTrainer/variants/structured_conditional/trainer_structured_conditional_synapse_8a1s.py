...
from .dataloader_structured_conditional_synapse_8a1s import StructuredConditionalDataLoader3D
from .inference_structured_conditional_synapse_8a1s import (
...
from .metrics_structured_conditional_synapse_8a1s import (
...
from .structured_loss_synapse_8a1s import StructuredConditionalLoss, StructuredLossConfig


class nnUNetTrainerStructuredConditionalSynapse8A1S(nnUNetTrainer):
    """
    Structured conditional trainer for Synapse 13-organ, 8-anchor / 1-slot variant:
    - one shared model
    - fixed 11-channel output head (8 anchors + 1 conditional slot + other)
    - 5 single-organ dynamic groups (gallbladder, esophagus, portal/splenic vein,
      r_adrenal, l_adrenal), each routed through the single conditional slot
