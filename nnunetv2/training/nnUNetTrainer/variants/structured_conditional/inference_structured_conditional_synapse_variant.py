...
def predict_logits_all_groups(
    ...
    return {
        group_id: predict_logits_for_group(network, image, group_id=group_id, use_amp=use_amp)
        for group_id in range(lm.NUM_DYNAMIC_GROUPS)
    }
...
def reconstruct_original_labels_from_all_groups(
    ...
    score_map = first_logits.new_full((b, lm.NUM_ORIGINAL_LABELS, *spatial), fill_value=-1e4)

    bg_scores = []
    fixed_scores = {int(v): [] for v in lm.FIXED_OUTPUT_TO_ORIGINAL.values()}
    for group_id, logits in logits_by_group.items():
        probs = torch.softmax(logits, dim=1)
        bg_scores.append(probs[:, 0])
        for output_channel, original_label in lm.FIXED_OUTPUT_TO_ORIGINAL.items():
            fixed_scores[int(original_label)].append(probs[:, int(output_channel)])
        for output_channel, original_label in lm.get_conditional_channel_to_original_label(int(group_id)).items():
    ...
    score_map[:, 0] = torch.stack(bg_scores, dim=0).mean(dim=0)
    return score_map.argmax(dim=1, keepdim=True).long(), score_map
