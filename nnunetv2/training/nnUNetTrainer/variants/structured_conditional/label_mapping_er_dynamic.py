...
# Fixed output head channels for 3-slot ER-dynamic variant.
...
def _load_dynamic_group_specs_from_json(grouping_config_path: str) -> Tuple[DynamicGroupSpec, ...]:
    ...
    if dataset_name in {"synapse", "btcv", "synapse13", "dataset210"}:
    ...
    anchors = tuple(config.get("anchors", ("ecs", "pm", "cyto", "nucpl")))
    if anchors != ("ecs", "pm", "cyto", "nucpl"):
        raise ValueError("CellMap AnchorSlot grouping config must keep anchors ['ecs', 'pm', 'cyto', 'nucpl']")
