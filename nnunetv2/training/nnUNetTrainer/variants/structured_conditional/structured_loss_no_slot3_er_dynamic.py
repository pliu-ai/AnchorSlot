...
class StructuredConditionalLoss(nn.Module):
    """
    Structured loss for fixed 9-channel ER-dynamic output with dynamic conditional slots.
    ...
    def _build_foreground_class_mask(active_conditional_slots: torch.Tensor, device: torch.device) -> torch.Tensor:
        ...
        # Fixed foreground channels. ER membrane/lumen are not fixed in this variant.
        for channel in (ECS_CHANNEL, PM_CHANNEL, CYTO_CHANNEL, NUCPL_CHANNEL, MITO_RIBO_CHANNEL):
            class_mask[:, channel] = True
