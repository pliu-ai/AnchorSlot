...
class StructuredConditionalLoss(nn.Module):
    """
    Structured loss for fixed 7-channel minimal-anchor output with dynamic conditional slots.
    ...
    def _build_foreground_class_mask(active_conditional_slots: torch.Tensor, device: torch.device) -> torch.Tensor:
        ...
        # Minimal anchors: only ecs, pm, and cyto are fixed foreground channels.
        for channel in (ECS_CHANNEL, PM_CHANNEL, CYTO_CHANNEL):
