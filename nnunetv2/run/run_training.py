...
def maybe_load_checkpoint(nnunet_trainer: nnUNetTrainer, continue_training: bool, validation_only: bool,
    ...
    if continue_training:
        requested_checkpoint = os.environ.get('NNUNET_CONTINUE_CHECKPOINT', '').strip()
        if requested_checkpoint:
            ...
            checkpoint_name = aliases.get(requested_checkpoint.lower(), requested_checkpoint)
            expected_checkpoint_file = checkpoint_name
            if not os.path.isabs(expected_checkpoint_file):
                expected_checkpoint_file = join(nnunet_trainer.output_folder, expected_checkpoint_file)
            if not isfile(expected_checkpoint_file):
                raise FileNotFoundError(
                    f"Requested continuation checkpoint does not exist: {expected_checkpoint_file}"
                )
            print(f"Continuing training from explicitly selected checkpoint: {expected_checkpoint_file}")
        else:
