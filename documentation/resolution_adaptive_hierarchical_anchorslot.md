...
reporting are performed only on the disjoint `imagesTs` cohorts (17 high-
resolution and 4 low-resolution volumes). The autogenerated crop-level nnU-Net
folds are useful for debugging, but not for paper claims because crops from the
same source specimen can occur on both sides of an internal fold.
